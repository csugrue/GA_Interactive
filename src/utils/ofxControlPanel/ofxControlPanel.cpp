...
void ofxControlPanel::loadFont( string fontName, int fontsize ){
    guiTTFFont.loadFont(fontName, fontsize,true,true);
	guiTTFFont.setLineHeight(fontsize+2);
...
void ofxControlPanel::setSelectedPanel( string panelName )
{
	for(int i = 0; i < panels.size(); i++){
		if( panels[i]->name == panelName ){
			selectedPanel = i;
...
guiTypeMultiToggle * ofxControlPanel::addMultiToggle(string name, string xmlName, int defaultBox, vector <string> boxNames){
    ...
    tmp->setDimensions(180, 17*boxNames.size());
    tmp->xmlName = xmlName;
...
//-------------------------------
guiTypeTextInput * ofxControlPanel::addTextInput(string sliderName, string xmlName, string value, int maxX, int maxY){
    if( currentPanel < 0 || currentPanel >= panels.size() )return NULL;
    ...
    //setup and dimensions
    tmp->setup(sliderName, value, maxX, maxY);
...
//---------------------------------------------
void ofxControlPanel::setValueS(string xmlName, string val, int whichParam){
    for(int i = 0; i < guiObjects.size(); i++)
	{
        if( xmlName == guiObjects[i]->name && guiObjects[i]->dataType == SG_TYPE_STRING )
		{
			guiObjects[i]->value.setValueAsStr(val);
        }
    }
...
string ofxControlPanel::getValueS(string xmlName, int whichParam){
    for(int i = 0; i < guiObjects.size(); i++){
        if( xmlName == guiObjects[i]->name && guiObjects[i]->dataType == SG_TYPE_STRING ){
            //if( whichParam >= 0 && whichParam < xmlObjects[i].numParams ){
                return guiObjects[i]->value.getValueS();
           // }
        }
    }
    ofLog(OF_LOG_WARNING, "ofxControlPanel - paramater requested %s doesn't exist - returning 0", xmlName.c_str());
	...
	for(int i = 0; i < guiObjects.size(); i++){
        if( guiObjects[i]->dataType == SG_TYPE_STRING && guiObjects[i]->value.getValueB() ){
            if( whichParam >= 0  ){
				...
				string nowStr = guiObjects[i]->value.getValueS();
...
void ofxControlPanel::deleteLastChar(int whichParam)
{
	for(int i = 0; i < guiObjects.size(); i++){
        if( guiObjects[i]->dataType == SG_TYPE_STRING  && guiObjects[i]->value.getValueB() ){
            if( whichParam >= 0  ){
				string nowStr = guiObjects[i]->value.getValueS();
				if( nowStr.size() > 0 ) nowStr.erase( nowStr.size()-1, nowStr.size() );
				guiObjects[i]->value.setValueAsStr(nowStr, whichParam);
				return;
			}
		}
...
//---------------------------------------------
bool ofxControlPanel::isAnyTextBoxActive(){
	for(int i = 0; i < xmlObjects.size(); i++){
		if( xmlObjects[i].guiObj->dataType == SG_TYPE_STRING && xmlObjects[i].guiObj->value.getValueB() ) 
			return true;
	}
	return false;
}
